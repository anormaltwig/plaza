...
use crate::bureau::BureauOptions;

use super::bureau_manager::BureauManager;
...
pub fn run(addr: SocketAddr, options: WlsOptions) -> io::Result<()> {
	...
	for wrl in wrls {
		managers.insert(
			wrl,
			BureauManager::new(options.max_bureaus, options.bureau_options),
		);
	}
